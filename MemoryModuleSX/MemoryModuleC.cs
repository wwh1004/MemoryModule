...
namespace MemoryModuleSX
{
    ...
    public static unsafe class MemoryModuleC
    {
        ...
        private static void* _tcslen(void* str)
        {
            if (Unicode)
        ...
        private static int _tcstol(void* str, void** endPtr, int radix)
        {
            if (Unicode)
...
#pragma warning disable IDE1006
        private static int lstrlen(void* str)
        {
            if (Unicode)
...
#pragma warning restore IDE1006
        ...
        internal static void* MemoryFindResource(MEMORYMODULE module, string name, string type)
        {
            return MemoryFindResourceEx(module, name, type, DEFAULT_LANGUAGE);
        }

        private static IMAGE_RESOURCE_DIRECTORY_ENTRY* _MemorySearchResourceEntry(void* root, IMAGE_RESOURCE_DIRECTORY* resources, byte* key)
        {
            ...
            if (!IS_INTRESOURCE(key) && key[0] == (byte)'#')
            {
                ...
                byte* endpos = null;
                int tmpkey = (ushort)_tcstol(&key[1], (void**)&endpos, 10);
            ...
            {
                char* searchKey;
                uint searchKeyLen = (uint)_tcslen(key);
        ...
        internal static void* MemoryFindResourceEx(MEMORYMODULE module, string s_name, string s_type, ushort language)
        {
            byte* name;
            byte* type;
            if (Unicode)
            ...
            return MemoryFindResourceEx(module, name, type, language);
        }

        internal static void* MemoryFindResourceEx(MEMORYMODULE module, byte* name, byte* type, ushort language)
        {
            byte* codeBase = module.codeBase;
            ...
            foundLanguage = _MemorySearchResourceEntry(rootResources, nameResources, (byte*)language);
        ...
        internal static uint MemorySizeofResource(MEMORYMODULE module, void* resource)
        {
        ...
        internal static void* MemoryLoadResource(MEMORYMODULE module, void* resource)
        {
            byte* codeBase = module.codeBase;
        ...
        internal static int MemoryLoadString(MEMORYMODULE module, uint id, out string buffer, int maxsize)
        {
            return MemoryLoadStringEx(module, id, out buffer, maxsize, DEFAULT_LANGUAGE);
        }

        internal static int MemoryLoadStringEx(MEMORYMODULE module, uint id, out string buffer, int maxsize, ushort language)
        {
            buffer = null;
            void* resource;
            ...
            fixed (char* p = new char[maxsize])
                _buffer = p;
            resource = MemoryFindResourceEx(module, (byte*)MAKEINTRESOURCE((id >> 4) + 1), (byte*)RT_STRING, language);
            if (resource == null)
                return 0;

            data = (IMAGE_RESOURCE_DIR_STRING_U*)MemoryLoadResource(module, resource);
            id = id & 0x0f;
            while (id-- != 0)
